...
def _populate(vault, settings_block="app:\n  mode: advanced\n"):
    ...
    (inbox / SETTINGS_FILE_NAME).write_text(
        f"# Plan Week Configuration\n\n```yaml\n{settings_block}```\n", encoding="utf-8")
...
def test_it_moves_nowspace_files_and_records_it(client, vault, monkeypatch):
    _fresh(monkeypatch)
    inbox = _populate(vault)

    r = client.post("/plan/move-plan-folder")
    assert r.status_code == 200, r.text
    assert r.json()["folder"] == MOVE_DESTINATION

    dest = vault / "5-Meta" / "Nowspace"
    for name in NOWSPACE_FILES + [SETTINGS_FILE_NAME]:
        assert (dest / name).exists(), f"{name} did not move"
        assert not (inbox / name).exists(), f"{name} left behind"

    # the setting is in the file at its NEW home, and resolves
    config._vault_cfg_cache = None
    config._vault_cfg_mtime = None
    assert config.plan_folder == MOVE_DESTINATION
    assert config.vault_path == dest
    assert config._vault_settings_path == dest / SETTINGS_FILE_NAME
