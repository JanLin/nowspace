...
export default function VaultBrowser({ onClose, stateRef, onOpenNote }: Props) {
  ...
  const [breadcrumb, setBreadcrumb] = useState<{ name: string; path: string }[]>(saved?.breadcrumb || []);
  ...
  const loadFolder = useCallback(async (path: string) => {
    ...
    try {
      ...
      // For root, show the PARA folders
      const fileList = path === "" ? res.files : res.files;
      setFiles(fileList);
      setFolderCache(prev => new Map(prev).set(path, fileList));
    } catch {
      setFiles([]);
    }
    setLoading(false);
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-3 pt-2 pb-1 border-t" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center gap-1 text-[10px] text-gray-400 flex-wrap">
            <button onClick={navigateRoot} className="hover:text-blue-500 transition-colors font-medium">
              Vault
            </button>
            {breadcrumb.map((b, i) => (
              <React.Fragment key={i}>
                <span>/</span>
                <button onClick={() => navigateBack(i)} className="hover:text-blue-500 transition-colors">
                  {b.name}
                </button>
              </React.Fragment>
            ))}
              ...
              {files.filter(f => f.type === "folder").map(f => {
                ...
                return (
                  <FolderRow
                    ...
                    file={f}
                    refName={refName}
                    isExpanded={expandedFolders.has(f.path)}
                    isDropTarget={dropTarget === f.path}
                    subFiles={subFiles.get(f.path)}
                    refNameInput={refNameInput}
                    onNavigate={() => navigateInto(f)}
...
function FolderRow({ file, refName, isExpanded, isDropTarget, subFiles, refNameInput, onNavigate, onToggleExpand, onStarClick, onRefNameChange, onRefNameSubmit, onRefNameCancel, onDragStart, onDragOver, onDragLeave, onDrop, onOpenFile, onDeleteFile, isPinned, onTogglePin }: FolderRowProps) {
  ...
  return (
    <div>
      ...
      >
        <button onClick={onToggleExpand} className="text-[9px] text-gray-400 w-3 shrink-0">
          {isExpanded ? "\u25BC" : "\u25B6"}
        </button>
        <span
      ...
      {isExpanded && subFiles && (
        <div className="ml-3 border-l border-gray-200 pl-1">
          ...
          {subFiles.filter(f => f.type === "folder").map(f => (
            <div
              key={f.path}
          ...
          {subFiles.filter(f => f.type === "file").map(f => (
